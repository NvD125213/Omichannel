...
const readOnlyTextareaClass =
  "min-h-28 resize-none rounded-[calc(0.75rem-0.125rem)] border-0 bg-transparent px-3 py-2 text-sm leading-relaxed text-foreground/88 shadow-none read-only:cursor-default read-only:opacity-100 focus-visible:ring-0";
...
const sectionHeaderClass =
  "border-b border-border/50 bg-muted/15 px-4 py-3.5 dark:bg-transparent";
...
function ConfigRow({
  ...
  return (
    <div className="px-4 py-3.5">
      <Label className="mb-2 block text-[10px] font-semibold tracking-[0.14em] text-muted-foreground/80 uppercase">
        {label}
      </Label>
      {multiline ? (
...
function OverviewMetaItem({
  ...
  return (
    <div className="flex min-w-0 items-start gap-3 px-4 py-3.5">
      {Icon && (
        <span className="mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-full border border-border/60 bg-muted/30 dark:bg-transparent">
      ...
      <div className="min-w-0 space-y-1">
        ...
        <p
          className={cn(
            "text-sm leading-relaxed break-all text-foreground/90",
  ...
  return (
    <section className={sectionShellClass}>
      <div className={sectionHeaderClass}>
        ...
        <p className="mt-1 text-xs leading-relaxed text-muted-foreground/80">
          Trạng thái hoạt động và key agent
        </p>
      </div>

      <div className="grid divide-border/50 sm:grid-cols-2 sm:divide-x">
        <div className="min-w-0 space-y-2.5 px-4 py-3.5">
          ...
          <Badge
            variant="outline"
            className={cn(
              "w-fit rounded-full px-2.5 py-0.5 text-xs font-medium tracking-wide",
...
function AgentConfigSections({ config }: { config: AgentConfig }) {
  const llmRows = renderConfigRows([
  ...
  const lightragRows = renderConfigRows([
  ...
  const promptRows = renderConfigRows([
    {
      label: "System prompt",
  ...
  const styleRows = renderConfigRows([
  ...
  const queryRewriteRows = renderConfigRows([
  ...
  const behaviorRows = renderConfigRows([
    {
  ...
  const retrievalRows = renderConfigRows([
  ...
  const businessScopeRows = renderConfigRows([
  ...
  const faqRows = renderConfigRows([
  ...
  const memoryRows = renderConfigRows([
...
export function AgentDataDetailPanel({ agent }: AgentDataDetailPanelProps) {
  ...
  return (
    <div className="space-y-5 dark:bg-transparent">
      {isLoading && !liveAgent && (
