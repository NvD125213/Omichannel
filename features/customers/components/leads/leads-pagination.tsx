...
interface LeadsDataPaginationProps<TData> {
  table: Table<TData>;
  pagination?: {
    total: number;
    page: number;
...
export function LeadsDataPagination<TData>({
  table,
  pagination,
  currentPage = 1,
  currentPageSize = 10,
  onPageChange,
  onPageSizeChange,
}: LeadsDataPaginationProps<TData>) {
  ...
  return (
    <div className="flex items-center justify-between px-4">
      <div className="hidden flex-1 text-sm text-muted-foreground lg:flex">
        {table.getFilteredSelectedRowModel().rows.length} bản ghi được chọn.
      </div>
      <div className="flex w-full items-center gap-8 lg:w-fit">
        <div className="hidden items-center gap-2 lg:flex">
          <Label htmlFor="leads-rows-per-page" className="text-sm font-medium">
